...
do
    ...
    $line | grep "Gflops\|GFLOPs\|busbw" -A 2 | tee ${fPerf}
    echo $line > ${fCmd}
    if [[ $id == "0" ]]; then
        cmd_metric="TestCommand"
        perf_metric=`head -n 1 ${fPerf} | sed -e 's/\r//g' |sed -e 's/#//g'| sed -e 's/^\s\s*//g' | sed -e 's/\s\s*/,/g'`
    ...
    perf_data=`tail -n 1 ${fPerf} | sed -e 's/\r//g' | sed -e 's/^\s\s*//g' | sed -e 's/\s\s*/,/g'`
    kernel_data=`cat ${fStats} | grep "nccl\|msccl\|rccl" | sed -e 's/\r//g'`
